...
public class CharDependencyReader {
	...
	public void InitZparFormat(String inputFile) throws Exception {
		if (inputFile == null)
			return;
		BufferedReader in = new BufferedReader(new InputStreamReader(
				new FileInputStream(inputFile), "UTF8"));
		...
		while ((sLine = in.readLine()) != null) {
			if (sLine.trim().equals("")) {
				...
				for (int i = 0; i < oneSentence.size(); i++) {
					...
					String cpostag = unit_labels[1] + "#" + unit_labels[2];
					String postag = cpostag;
																				
					tmpInstance.cpostags.add(cpostag);
					tmpInstance.postags.add(postag);
					
					tmpInstance.feats.add("");
										
					if(unit_labels[3].equals("_"))
					{
						unit_labels[3] = "-2";
					}
					
					tmpInstance.heads.add(Integer.parseInt(unit_labels[3])+1);
					if(unit_labels[5].equals("in")){
						tmpInstance.deprels.add("#in");
					}
					else{
						tmpInstance.deprels.add(unit_labels[4]);
					}
					...
					tmpInstance.p1heads.add(-1);
					tmpInstance.p1deprels.add("");					
