...
public class ExtractTSTranslationRules {
	...
	public static void main(String[] args) throws Exception{
		...
		int totalitems = target2source.size();
		int onebase = totalitems/100;
		int processeditem = 0;
		Map<String, Integer> sourceCandidates = new HashMap<String, Integer>();
		for(String targetstr : target2source.keySet())
		{
			processeditem++;
			if(target2source.get(targetstr).size() > maxitem)
			{
				maxitem = target2source.get(targetstr).size();
			}
			sourceCandidates.put(targetstr, target2source.get(targetstr).size());
			allitem = allitem + target2source.get(targetstr).size();
			for(String sourcestr : target2source.get(targetstr).keySet())
			{
				if(Math.abs(target2source.get(targetstr).get(sourcestr) - source2target.get(sourcestr).get(targetstr)) > 0.5)
				{
					System.out.println("error, count not match!");
				}
				float weight1 = (float)Math.log(target2source.get(targetstr).get(sourcestr)/targetFreqs.get(targetstr));
				float weight2 = (float)Math.log(target2source.get(targetstr).get(sourcestr)/sourceFreqs.get(sourcestr));
				float weight3 = (float)Math.log(target2source.get(targetstr).get(sourcestr)/overallFreq);
				...
				String target = TranslationRulesGeneration.theSaveWordString(targetwords);
				trg.AddOneRule(target, sourcewords, weight2, weight1, weight3, ruleType);
