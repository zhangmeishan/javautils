...
public class DepInstance {
	...
	public boolean evaluateWithOtherSANCL(DepInstance other, List<Integer> uas_results, List<Integer> las_Results) {
		for (int i = 0; i < 8; i++) {
			eval_res[i] = 0;
		}

		uas_results.clear(); las_Results.clear();
		int length = forms.size();
		
		if(other.forms.size() != length)return false;
		for (int i = 0; i < length; i++) {
			if (isPunc(forms.get(i), cpostags.get(i)) || isPunc(forms.get(i), postags.get(i))) {
				uas_results.add(-1); las_Results.add(-1);
				continue;
			}
			eval_res[0]++;
			int curHead = heads.get(i);
