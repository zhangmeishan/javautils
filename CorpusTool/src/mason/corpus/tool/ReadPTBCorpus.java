...
public class ReadPTBCorpus {
	...
	{		
		...
		while (treeReader.hasNext()) {
			...
			//only POS
			List<Tree<String>> poswords = normalizedTree.getPreTerminals();
			String outpos = bFolderMArk + ":";
			for(int idx = 0; idx < poswords.size(); idx++){
				
			}
			writer.println(bFolderMArk + ":" + normalizedTree.toString());
			writer.flush();
			iCount++;
		}
