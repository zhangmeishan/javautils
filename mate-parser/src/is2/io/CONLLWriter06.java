...
//	public int version = CONLLReader09.TASK08;
	...
	public void write(SentenceData09 inst) throws IOException {
		...
		for (int i=0; i<inst.length(); i++) {
				...
				writer.write(inst.gpos[i]);	// cpos
				writer.write('\t');
				
				writer.write(inst.ppos[i]); // gpos
				writer.write('\t');  
				...
				String headstr = Integer.toString(inst.heads[i][0]);
				for(int r=1; r<inst.heads[i].length;r++) headstr=headstr+"|"+Integer.toString(inst.heads[i][r]);
				writer.write(headstr);  writer.write('\t');  // head
