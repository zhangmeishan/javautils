...
public class Evaluator {
	...
	public static Results evaluate (String act_file,  String pred_file) throws Exception {
		...
		CONLLReader06 goldReader = new CONLLReader06(act_file, -1);
		CONLLReader06 predictedReader = new CONLLReader06(pred_file, -1);
		...
		while(goldInstance != null) {
			...
			String[] gposLabels = goldInstance.gpos;
			...
			for (int i = 1; i < instanceLength; i++) {
				...
				if (predHeads[i][0] == goldHeads[i][0]) {
					corr++;
