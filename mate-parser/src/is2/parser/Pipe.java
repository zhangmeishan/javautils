...
final public class Pipe extends PipeGen {
	...
	throws Exception {
		...
		CONLLReader06 depReader = new CONLLReader06(file);
		...
		while (true) {
			...
			String[] plabs1 = instance.plabels;
			for (int i1 = 0; i1 < plabs1.length; i1++) mf.register(REL, plabs1[i1]);
		...
		while (true) {
			...
			for (int k = 0; k < is.length(last); k++) {
				for(int s=0; s<is.heads[last][k].length;s++)
				{
					if (is.heads[last][k][s] < 0)	continue;
					Edges.put(pos[is.heads[last][k][s]],pos[k], k < is.heads[last][k][s],is.labels[last][k]);
				}
			}
	 ...
	 * @throws IOException
	 */
	protected final SentenceData09 nextInstance(Instances is, CONLLReader06 depReader) throws Exception {
...
*/	
	public double errors( Parse g, Parse p) {
		short[] act = g.heads;
		int correct = 0;
		...
		for(int i = 1; i < act.length; i++)  	 {
			...
			//		if (is.ppos[ic] ==null ) System.out.println("mf null"+is.ppos[ic][i]);
			if (p.heads[i]==act[i] && p.labels[i]==g.labels[i] ) correct++;
		}
