...
//			DB.println("misses "+LongIntHash.misses+" good "+LongIntHash.good);
	...
	throws Exception{
		...
		for(; iter < options.numIters; iter++) {
			...
			for(int n = 0; n < numInstances; n++) {
				...
				{
					Random rand = new Random(iter*numInstances+n);
					double pi = rand.nextDouble();
					int length =  is.heads[n].length-1;
					double vals = pi*length*(length+1)/2;
					int wordcount = 1;
					double sum = 1.0;
					while(sum < vals)
					{
					...
					//System.out.println(String.format("%d of %d", wordcount, length));
					List<Integer> indexes = new ArrayList<Integer>();
					for(int r=1; r<is.heads[n].length;r++)indexes.add(r);
					Collections.shuffle(indexes, rand);
					wordcount =  length;
					for(int t=0; t<wordcount;t++)
					{
						int r = indexes.get(t);
						Set<Short> candidates = new HashSet<Short>();
						for(int s=0; s<is.heads[n][r].length;s++)
						{
							candidates.add(is.heads[n][r][s]);
						}
						
						for(short s=0; s<is.heads[n].length;s++)
						{
							if(!candidates.contains(s))
							{
								data.pl[s][r] = -Float.MAX_VALUE;
							}
						}
					}
				}
								
				Parse g = Decoder.decode(pos,  data, options.decodeProjective);

				double e= pipe.errors(g, d);
				...
				act.clear();
				pipe.extractor[0].encodeCat(pos,is.forms[n],is.plemmas[n],g.heads, g.labels, is.feats[n], is.pheads[n], is.plabels[n], act);
	...
	throws Exception {
		...
		while(true) {
			...
			//add constrains
			for(int r=1; r<is.heads[0].length;r++)
			{
				Set<Short> candidates = new HashSet<Short>();
				for(int s=0; s<is.heads[0][r].length;s++)
				{
					candidates.add(is.heads[0][r][s]);
				}
				
				for(short s=0; s<is.heads[0].length;s++)
				{
					if(!candidates.contains(s))
					{
						d.pl[s][r] = -Float.MAX_VALUE;
					}
				}
			}
			...
			int[][] heads = new int[formsNoRoot.length][1];
	...
	throws Exception {
		...
		while(true) {
			...
			for (int i = 0; i < i09.length(); i++) {
				...
				String headstr = Integer.toString(i09.heads[i][0]);
				for(int r=1; r<i09.heads[i].length;r++) headstr=headstr+"|"+Integer.toString(i09.heads[i][r]);
				outWriter.write(headstr);
	...
	throws Exception {
		...
		while(true) {
			...
			short[] goldHeads = new short[is.heads[0].length];
			for(int s=0; s<goldHeads.length;s++)goldHeads[s]=is.heads[0][s][0];
