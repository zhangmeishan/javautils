...
public class SentenceData09 {
	...
	public int[][] heads; //only heads has candidate, not perfect, should labels also be!
	public int[] pheads;
...
//	public String[] split_lemma;
	...
	public SentenceData09(String[] forms, String[] olemmas, String[] lemmas,String[] gpos, 
		...
		labels = labs;
		this.heads = heads;
		this.pheads = new int[heads.length];
		for(int r=0; r<heads.length;r++)this.pheads[r]=heads[r][0];
		this.plabels=labs;
		this.plemmas = lemmas;
		this.lemmas =olemmas;
	 ...
	 * @param instance
	 */
	public SentenceData09 Copy() {
		
		SentenceData09 newInst = new SentenceData09();
		int length = forms.length;
		
		newInst.forms = new String[length];
		newInst.gpos = new String[length];
		newInst.ppos = new String[length];
		newInst.plemmas = new String[length];
		newInst.plabels = new String[length];
		newInst.lemmas = new String[length];
		newInst.heads = new int[length][];
		newInst.pheads = new int[length];
		newInst.ofeats = new String[length];
		newInst.pfeats = new String[length];
		newInst.labels = new String[length];
		newInst.fillp = new String[length];
		newInst.id = new String[length];
		
		
		for(int j = 0; j < length; j++) {
			newInst.forms[j] = forms[j];
			newInst.ppos[j] = ppos[j];
			newInst.gpos[j] = gpos[j];

			newInst.labels[j] = labels[j];
			newInst.heads[j] = heads[j];
			if (pheads!=null) newInst.pheads[j] = pheads[j];
			if (lemmas!=null) newInst.lemmas[j] = lemmas[j];

			newInst.plemmas[j] = plemmas[j];
			

			if (ofeats!=null)  newInst.ofeats[j] = ofeats[j];
			if (pfeats!=null)	newInst.pfeats[j] = pfeats[j];

			if (fillp!=null) newInst.fillp[j] = fillp[j];
			if (id!=null) newInst.id[j] = id[j];
		}
	...
	/*
		...
		for(int k=0;k<forms.length;k++) {
			...
			out.writeUTF(ppos[k]);
			out.writeUTF(gpos[k]);
			for(int r=0; r<heads[k].length;r++)
			{
				out.writeInt(heads[k][r]);
			}
